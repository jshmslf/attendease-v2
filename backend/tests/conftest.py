...
async def _delete_test_data(session):
    ...
    # student_id is now auto-generated by the API, so test rows are identified by
    # their distinctive ".test@school.edu.ph" email domain instead.
    result = await session.execute(
        select(Student.id).where(Student.email.like("%.test@school.edu.ph"))
    )
    ...
    await session.execute(delete(Student).where(Student.email.like("%.test@school.edu.ph")))
