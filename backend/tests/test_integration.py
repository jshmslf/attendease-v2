...
# student_id is now auto-generated by the API (e.g. "2026-00001"), so it can no
# longer be set by the test payload. Tests that need it capture the value
# returned by test_create_student and reuse it via this shared dict.
_student_ids = {}
...
class TestStudents:
    async def test_create_student(self, client: AsyncClient, admin_token: str):
        ...
        _student_ids["main"] = data["student_id"]
    ...
    async def test_get_student_by_id(self, client: AsyncClient, admin_token: str):
        response = await client.get(
            f"/api/students/{_student_ids['main']}",
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        assert response.status_code == 200
        assert response.json()["student_id"] == _student_ids["main"]
...
class TestStudentPortal:
    async def test_create_portal_account(self, client: AsyncClient, admin_token: str):
        response = await client.post(
            f"/api/students/{_student_ids['main']}/portal-account",
    ...
    async def test_student_login_success(self, client: AsyncClient):
        response = await client.post(
            ...
            json={"student_id": _student_ids["main"], "password": "Student@1234"},
        )
        assert response.status_code == 200
    ...
    async def test_student_login_wrong_password(self, client: AsyncClient):
        response = await client.post(
            "/api/auth/student/login",
            json={"student_id": _student_ids["main"], "password": "wrong"},
        )
        assert response.status_code == 401
